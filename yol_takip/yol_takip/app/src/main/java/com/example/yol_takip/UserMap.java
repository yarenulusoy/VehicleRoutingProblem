...
public class UserMap extends AppCompatActivity implements OnMapReadyCallback, TaskLoadedCallback {
    GoogleMap mMap;
    Button getDirection;
    int deger=0;
    ...
    protected void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        DocumentReference docRef2 = db.collection("rotalar").document(data);
            ...
            public void onComplete(@NonNull Task<DocumentSnapshot> task) {
                if (task.isSuccessful()) {
                    ...
                    if (document.exists()) {
                        ...
                        DocumentReference docRef = db.collection("talepler").document("2");
                            ...
                            public void onComplete(@NonNull Task<DocumentSnapshot> task) {
                                if (task.isSuccessful()) {
                                    ...
                                    if (document.exists()) {
                                        ...
                                        String konum=document.getData().values().toString();

                                        db.collection("yoltakip").whereNotEqualTo("durakAdi",konum)
                                                .get()
                                                    ...
                                                    public void onComplete(@NonNull Task<QuerySnapshot> task) {
                                                        if (task.isSuccessful()) {
                                                            for (QueryDocumentSnapshot document : task.getResult()) {
                                                                ...
                                                                String subject = durak.getLatitude();
                                                                for(int i=0;i<rotalar.size();i++){
                                                                    if(Double.valueOf(subject)==Double.valueOf(rotalar.get(i))){
                ...
                for (int i = 0; i < rotalar.size()-2; i += 2) {
                    if(i!=0){
                        if(rotalar.get(i)==40.823371 || rotalar.get(i+2)==40.823371) {
                            ...
                            i=i+2;
                            new FetchURL(UserMap.this).execute(getUrl(new MarkerOptions().position(new LatLng(rotalar.get(i), rotalar.get(i+1))).
